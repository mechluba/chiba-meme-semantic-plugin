    ...
    def build_semantic_selector_prompt(
        ...
        *,
        ...
        return (
            ...
            "判定顺序：\n"
            "0. 先核对当前回复任务与最近已发送的助手回复。旧视频、旧任务和助手自己用过的梗"
            "不能充当当前场景的触发证据；用户指出重复或要求停用时选 SKIP。最近一两条助手回复"
            "已经使用明显的梗或固定句式时，除非用户明确要求引用或重复，否则选 SKIP；不能通过换梗连续接力。\n"
            ...
            "否则 SKIP。\n"
            "C. 0/A/B 通过、route 明确吻合且能增加私人轻松对话的社交表达价值时，"
            "积极选择 USE。不得仅因原句夸张、用户没提作品名/圈层名或"
            "最终角色措辞而拒绝；Replyer 会自然改写。\n"
