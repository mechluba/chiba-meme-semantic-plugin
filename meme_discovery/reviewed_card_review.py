...
<script>
const doc=JSON.parse(document.getElementById('dataset').textContent);const KEY='chiba-reviewed-card-decisions-web-grounded-v2';
...
function researchHTML(x){{const w=x.web_research||{{}},k=x.storage_card?.knowledge?.source_material?.web_research||{{}},sources=w.results||[];const list=sources.map(v=>`<div class="source"><a href="${{esc(safeURL(v.url))}}" target="_blank" rel="noopener noreferrer">${{esc(v.title||v.url)}}</a><div class="source-meta">${{esc(v.provider)}}${{v.published_at?' · '+esc(v.published_at.slice(0,10)):''}} · ${{esc(v.match_quality||'')}}</div>${{v.snippet?`<div>${{esc(v.snippet)}}</div>`:''}}</div>`).join('');return `<div class="research"><div><b>典故 / 来源</b><div>${{esc(k.origin_summary||'来源待核')}}</div></div><div><b>当前用法</b><div>${{esc(k.current_usage_summary||'待核')}}</div></div></div><div class="muted">检索：${{esc(researchLabels[w.status]||w.status||'来源待核')}} · 时效：${{esc(k.freshness_assessment||w.freshness?.class||'unknown')}} · 可信度：${{esc(k.research_confidence||'low')}}</div><details><summary>互联网检索材料 ${{sources.length}} 条</summary>${{list||'<div class="muted">未找到可靠外部命中，仅保留弹幕证据，需人工补充来源。</div>'}}</details>`}}
...
document.getElementById('search').oninput=render;document.getElementById('policy').onchange=render;document.getElementById('research').onchange=render;document.getElementById('decision').onchange=render;save();render();
</script>
</body></html>"""
