...
@media(max-width:700px){{header{{position:static}} .routes{{grid-template-columns:1fr}} input[type=search]{{min-width:100%}}}}
</style>
</head>
<body>
<header><div class="top"><h1>千叶热梗语义卡二审</h1><span class="stat">生成 {generated} · 失败 {failed}</span><span id="progress" class="stat"></span><button class="primary" onclick="exportLibrary()">导出审核后梗库</button><button onclick="exportDecisions()">导出审核记录</button><button onclick="exportServing()">导出仅理解配置</button><button onclick="document.getElementById('importer').click()">导入审核记录</button><input id="importer" type="file" accept="application/json" hidden></div>
<div class="filters"><input id="search" type="search" placeholder="搜索表达、语义、交流意图、圈层"><select id="policy"><option value="all">全部策略</option><option value="refine">可使用</option><option value="understand">仅理解</option></select><select id="decision"><option value="all">全部审核状态</option><option value="pending">待审核</option><option value="approve_use">通过·可使用</option><option value="approve_understand">通过·仅理解</option><option value="reject">淘汰</option></select></div></header>
<main id="cards"></main>
<script id="dataset" type="application/json">{payload}</script>
<script>
const doc=JSON.parse(document.getElementById('dataset').textContent);const KEY='chiba-reviewed-card-decisions-v1';
...
function cardHTML(x){{const c=x.storage_card,s=state[c.card_id],routes=(c.usage_routes||[]).map(r=>`<div class="route"><b>${{esc(r.route_tag)}}</b><div>${{esc(r.when)}}</div><div class="muted">交流意图：${{esc(r.communicative_intent)}}</div><div class="muted">用法：${{esc((r.allowed_realizations||[]).join(' / '))}}</div></div>`).join('');return `<article id="a-${{c.card_id}}" data-policy="${{x.prior_serving_policy}}" data-decision="${{s.decision}}"><div class="row"><div class="grow"><h2>${{esc(c.canonical_expression)}} <span class="badge">${{esc(x.content_type)}}</span> <span class="badge ${{x.prior_serving_policy==='understand'?'understand':'use'}}">${{x.prior_serving_policy==='understand'?'初审仅理解':'初审可使用'}}</span></h2><div class="muted">${{esc((c.aliases||[]).join(' · '))}}</div></div><select aria-label="审核决定" onchange="setDecision('${{c.card_id}}',this.value)"><option value="pending" ${{s.decision==='pending'?'selected':''}}>待审核</option><option value="approve_use" ${{s.decision==='approve_use'?'selected':''}}>通过·可使用</option><option value="approve_understand" ${{s.decision==='approve_understand'?'selected':''}}>通过·仅理解</option><option value="reject" ${{s.decision==='reject'?'selected':''}}>淘汰</option></select><button onclick="acceptSuggestion('${{c.card_id}}','${{initialDecision(x)}}')">采用初审建议</button></div><div class="core"><b>真实表达：</b>${{esc(c.semantic_core)}}</div><div class="muted">圈层：${{esc(c.culture_scope)}} · 投放范围：${{esc(c.serving_scope)}}</div><div class="routes">${{routes}}</div><div class="row"><div class="grow"><b>触发信号：</b>${{esc((c.required_context_signals||[]).join('；'))}}</div><div class="grow"><b>硬禁用：</b>${{esc((c.hard_blocks||[]).join('；'))}}</div></div><details><summary>存储内容 JSON（已折叠，可直接编辑）</summary><textarea spellcheck="false" oninput="editJson('${{c.card_id}}',this.value)" onblur="checkJson('${{c.card_id}}')">${{esc(s.json)}}</textarea><div class="json-tools"><button onclick="formatJson('${{c.card_id}}')">格式化并校验</button><button onclick="resetJson('${{c.card_id}}')">恢复模型版本</button><span id="msg-${{c.card_id}}"></span></div></details></article>`}}
function render(){{const q=document.getElementById('search').value.trim().toLowerCase(),p=document.getElementById('policy').value,d=document.getElementById('decision').value;const shown=items.filter(x=>{{const c=x.storage_card,s=state[c.card_id];return(p==='all'||x.prior_serving_policy===p)&&(d==='all'||s.decision===d)&&(!q||JSON.stringify(c).toLowerCase().includes(q))}});document.getElementById('cards').innerHTML=shown.length?shown.map(cardHTML).join(''):'<div class="empty">没有符合条件的卡片</div>';updateProgress()}}
...
document.getElementById('search').oninput=render;document.getElementById('policy').onchange=render;document.getElementById('decision').onchange=render;save();render();
</script>
</body></html>"""
